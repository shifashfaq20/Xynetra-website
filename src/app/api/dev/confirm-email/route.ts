...
export async function GET(request: Request) {
  ...
  if (!serviceKey || !supabaseUrl) {
    return NextResponse.json(
      { error: "SUPABASE_SERVICE_ROLE_KEY or SUPABASE_URL not set" },
      { status: 500 }
    );
  }
  ...
  if (user.email_confirmed_at) {
    return NextResponse.json({ message: `User ${email} is already confirmed.` });
  }

  const { error: confirmErr } = await supabase.auth.admin.updateUserById(user.id, {
    email_confirm: true,
  });
