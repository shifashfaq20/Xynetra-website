...
/*
  These types fix the Netlify/TypeScript error.

  connectWhatsAppEmbedded can return either:
  - an error: { error: "..." }
  - a successful WhatsApp connection with display_name and number.
*/
type WhatsAppConnectSuccess = {
  display_name?: string | null
  number?: string | null
}

type WhatsAppConnectError = {
  error: string
}

type WhatsAppConnectResult =
  | WhatsAppConnectSuccess
  | WhatsAppConnectError

export function Step3ConnectWhatsApp({ initial, onNext }: Props) {
  const [status, setStatus] = useState<WhatsAppStatus>(
    initial.whatsapp_status ?? 'not_connected'
  )

  const [displayName, setDisplayName] = useState(
    initial.whatsapp_display_name || ''
  )
  ...
  // Assisted-form state
  const [option, setOption] = useState<PhoneOption | null>(
    initial.phone_option ?? null
  )
  ...
  // Load the official Meta sign-up SDK once — only when configured.
  useEffect(() => {
    if (!EMBEDDED_CONFIGURED) return
    ...
    script.onerror = () => {
      console.error('Sign-up SDK failed to load (ad blocker?)')
    }

    document.body.appendChild(script)
  }, [])

  // The popup posts the connected number IDs back to the app.
  useEffect(() => {
    function onMessage(event: MessageEvent) {
      if (!event.origin.includes('facebook.com')) return
      ...
      try {
        ...
        // Not a Meta/WhatsApp sign-up message.
      }
    }
  ...
  // FB.login wrapped so it can never leave the UI loading forever.
  function launchSignup(): Promise<string | null> {
    return new Promise((resolve) => {
      let settled = false
      ...
      const timer = setTimeout(() => done(null), 120000)

      try {
        window.FB.login(
          (response: any) => {
            done(response?.authResponse?.code ?? null)
          },
          {
            config_id: META_CONFIG_ID,
            response_type: 'code',
  ...
  // Wait briefly because the FINISH event can arrive just after login.
  async function waitForIds(timeoutMs = 5000) {
    const start = Date.now()

    while (Date.now() - start < timeoutMs) {
      if (pendingRef.current.phone_number_id && pendingRef.current.waba_id) {
        break
      }
  ...
  async function handleConnect() {
    ...
    try {
      ...
      if (!code) {
        setError('Sign-up was closed before finishing.')
        return
      }
      ...
      /*
        This "as WhatsAppConnectResult" is the important fix.
        It tells TypeScript that the server action can return
        either an error or successful WhatsApp details.
      */
      const res = (await connectWhatsAppEmbedded({
        code,
        waba_id: ids.waba_id,
        phone_number_id: ids.phone_number_id,
      })) as WhatsAppConnectResult

      /*
        Important:
        Do NOT use "if (res.error)" here.

        Successful responses do not have an "error" property.
        This check safely detects error responses first.
      */
      if ('error' in res) {
        setError(res.error)
        return
      ...
      setError('Could not connect WhatsApp. Please try again.')
    } finally {
      setBusy(false)
    }
  }
  ...
  async function handleDisconnect() {
    if (
      !confirm(
        'Disconnecting stops all reminders and slot recovery. Continue?'
      )
    ) {
      return
    }
  ...
  async function handleAssisted(e: React.FormEvent) {
    ...
    try {
      const res = await saveAssistedNumber({
        ...
        phone_country: option === 'agency_virtual' ? country : undefined,
        phone_area_code:
          option === 'agency_virtual' ? areaCode || undefined : undefined,
        phone_auth_confirmed:
          option === 'agency_virtual' ? authOk : undefined,
      })
  ...
  if (status === 'connected') {
    return (
      <div className="mx-auto max-w-lg space-y-6">
        ...
        <div className="space-y-1 rounded-xl border border-coral/40 bg-coral/5 p-5">
          <p className="text-xs font-semibold uppercase tracking-wider text-coral">
            Connected
          </p>

          <p className="text-lg font-semibold text-white">
            {displayName || 'Your business line'}
          </p>
          ...
          <p className="pt-2 text-xs text-zinc-500">
            Confirm this shows your exact business name — it&apos;s what your
            customers will see.
          </p>
        </div>
        ...
        <div className="flex gap-3">
          <button
            onClick={onNext}
            className="flex-1 rounded-lg bg-coral py-3 font-semibold text-white transition hover:brightness-110"
          >
            Continue
          </button>
          ...
          <button
            ...
            disabled={busy}
            className="rounded-lg border border-zinc-700 px-4 py-3 text-sm text-zinc-400 transition hover:border-red-900 hover:text-red-400"
          >
            Disconnect
          </button>
        </div>
  ...
  if (status === 'pending' && mode === 'main') {
    return (
      <div className="mx-auto max-w-lg space-y-6">
        ...
        <div className="space-y-1 rounded-xl border border-amber-500/40 bg-amber-500/5 p-5">
          <p className="text-xs font-semibold uppercase tracking-wider text-amber-300">
            Pending verification
          </p>

          <p className="text-sm text-zinc-300">
            We&apos;re setting up your dedicated number. This usually completes
            within one business day — you can finish the rest of onboarding
            now.
          </p>

          {initial.phone_number && (
            <p className="pt-1 text-sm text-zinc-400">
              Number: {initial.phone_number}
            </p>
          )}
        </div>
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            className="rounded-lg border border-zinc-700 px-4 py-3 text-sm text-zinc-400 transition hover:bg-zinc-800"
          >
            Change
          </button>
        </div>
  ...
  /* ── ASSISTED FORM ── */
  if (mode === 'assisted') {
    const optCls = (selectedOption: PhoneOption) =>
    ...
    return (
      ...
      >
        ...
        <p className="text-sm text-zinc-400">
          ...
          <strong className="text-zinc-200">your</strong> business identity
          and stays yours — even if you leave. It must be a fresh number not
          active on WhatsApp.
        </p>
        ...
        <div className="space-y-3">
          <button
            type="button"
            onClick={() => setOption('client_sim')}
            className={optCls('client_sim')}
          >
            <p className="text-sm font-semibold text-white">
              I&apos;ll get a local SIM{' '}
              <span className="ml-2 text-xs font-normal text-coral">
                Recommended
              </span>
            </p>

            <p className="mt-1 text-xs text-zinc-400">
              Cheapest prepaid SIM in the business&apos;s name. Don&apos;t
              install WhatsApp on it — we&apos;ll read you a one-time code on a
              short call.
            </p>
          </button>
          ...
          <button
            type="button"
            onClick={() => setOption('landline')}
            className={optCls('landline')}
          >
            <p className="text-sm font-semibold text-white">
              Use my business landline
            </p>

            <p className="mt-1 text-xs text-zinc-400">
              You&apos;ll answer one automated call to hear the code.
            </p>
          </button>

          <button
            type="button"
            onClick={() => setOption('agency_virtual')}
            className={optCls('agency_virtual')}
          >
            <p className="text-sm font-semibold text-white">
              Procure one for me{' '}
            ...
            <p className="mt-1 text-xs text-zinc-400">
              We arrange a number under your business identity. Transferable to
              you at any time.
            </p>
          </button>
        </div>
        ...
        {option === 'agency_virtual' && (
          <div className="space-y-4">
            <select
              ...
              className="w-full rounded-lg border border-zinc-700 bg-zinc-800 px-4 py-2.5 text-white outline-none focus:border-coral"
            >
              <option value="">Country for the number…</option>
            ...
            {['AE', 'SA', 'PK'].includes(country) && (
              <p className="rounded-lg border border-amber-500/30 bg-amber-500/10 p-3 text-xs text-amber-300/90">
                Virtual numbers verify unreliably in this country — a local SIM
                is cheaper and works every time.
              </p>
            )}
            ...
            <input
              ...
              className="w-full rounded-lg border border-zinc-700 bg-zinc-800 px-4 py-2.5 text-white placeholder-zinc-500 outline-none focus:border-coral"
              placeholder="Preferred area / city code (optional)"
            />
            ...
            <label className="flex cursor-pointer items-start gap-3 text-sm text-zinc-300">
              ...
              I authorize Xynetra to procure a number on my behalf, registered
              under my business identity.
            </label>
          </div>
        )}
        ...
        <div className="flex gap-3">
          ...
          <button
            type="submit"
            disabled={!assistedOk || busy}
            className="flex-1 rounded-lg bg-coral py-3 font-semibold text-white transition hover:brightness-110 disabled:bg-zinc-700 disabled:text-zinc-500"
          >
            {busy ? 'Saving…' : 'Save & Continue'}
          </button>
        </div>
  ...
  return (
    <div className="mx-auto max-w-lg space-y-6">
      ...
      <div className="space-y-1.5 rounded-lg border border-zinc-700 bg-zinc-800/60 p-4 text-xs text-zinc-400">
        <p>
          • You get a{' '}
          <strong className="text-zinc-200">dedicated number</strong> shown as
          your business — never a shared one.
        </p>

        <p>
          • The number and business account{' '}
          <strong className="text-zinc-200">belong to you</strong>. You connect
          through the official sign-up flow — we never see your password, and
          you can revoke access anytime.
        </p>
      </div>

      <div className="rounded-lg border border-amber-500/40 bg-amber-500/5 p-4 text-xs text-amber-200/90">
        <strong>Important:</strong> use a new number that is not currently
        active on WhatsApp. Registering an in-use number will disconnect it
      ...
      {EMBEDDED_CONFIGURED ? (
        <button
          ...
          className="w-full rounded-lg bg-coral py-3 font-semibold text-white transition hover:brightness-110 disabled:bg-zinc-700 disabled:text-zinc-500"
        >
          {busy
        ...
        <div className="rounded-lg border border-zinc-700 bg-zinc-800/50 p-4 text-sm text-zinc-400">
          One-click connection is being finalized on our side. For now, use the
          assisted option below — it takes about one business day.
        </div>
      )}

      <button
        onClick={() => setMode('assisted')}
        className="w-full text-center text-sm text-zinc-500 underline transition hover:text-zinc-300"
      >
        Can&apos;t arrange a number yourself? We&apos;ll help you get one.
      </button>
    </div>
