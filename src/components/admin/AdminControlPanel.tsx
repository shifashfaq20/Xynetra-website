...
// import { Building2, SlidersHorizontal, ExternalLink, Plus, FlaskConical, Users, Phone } from "lucide-react";
// import type { AdminClient } from "@/lib/admin/actions";
// import {
//   listClients,
//   updateClientSettings,
//   setClientPhoneNumberId,
//   createClientAccount,
...
//                     )}
//                   </div>
//                   <p className="mt-0.5 font-body text-sm text-ink/50">Owner Email: {c.email}</p>
...
//                 placeholder="e.g. 741852096312458"
//               />
//             </div>
//           </Field>
...
import type { AdminClient } from "@/lib/admin/roles";
import {
  listClients,
  updateClientSettings,
  setClientPhoneNumberId,
...
export function AdminControlPanel({
  ...
  const [notice, setNotice] = useState<{ kind: "ok" | "err"; msg: string } | null>(
    loadError ? { kind: "err", msg: loadError } : null
  );
  const [pending, start] = useTransition();
  ...
  useEffect(() => {
    ...
    active?.whatsapp_phone_number_id,
  ]);
  ...
  const reload = (preferId?: string) =>
    start(async () => {
      ...
      setClients(res.data);
      const target = preferId ?? activeId;
      setActiveId(res.data.find((c) => c.id === target)?.id ?? res.data[0]?.id ?? null);
    });
  ...
  const onSaveSettings = () => {
    ...
    setNotice(null);
    start(async () => {
      const a = await updateClientSettings(active.id, {
        billing_region: region,
        subscription_status: status,
      });
      if (!a.ok) {
        setNotice({ kind: "err", msg: a.error });
        return;
      }

      const b = await setClientPhoneNumberId(active.id, phoneNumberId.trim() || null);
      if (!b.ok) {
        setNotice({ kind: "err", msg: b.error });
        return;
      }
  ...
  const onAddClient = (e: FormEvent) => {
    ...
    start(async () => {
      ...
      setNotice({ kind: "ok", msg: `Created ${email}. They can log in now.` });
      setNewEmail("");
      setNewBiz("");
      setNewPass(genPassword());
      setShowAdd(false);
      reload(res.data.userId);
    });
  };
  ...
  const onSimulate = () => {
    ...
    start(async () => {
      ...
      setNotice(
        res.ok
          ? { kind: "ok", msg: `Simulation data written for ${active.business_name}.` }
  ...
  return (
    <div className="space-y-6">
      {notice && (
        <div
          className={`flex items-start justify-between gap-4 rounded-xl border px-4 py-3 text-sm font-body ${
            notice.kind === "ok"
              ? "border-emerald-300 bg-emerald-50 text-emerald-800"
              : "border-red-300 bg-red-50 text-red-700"
          }`}
              ...
              >
                <div>
                  ...
                  <div className="mt-2 flex flex-wrap gap-2">
                    ...
                    <Badge>Status: {c.subscription_status}</Badge>
                    <span
                      className={`rounded-md px-2.5 py-1 font-body text-xs font-semibold ${
                        linked ? "bg-emerald-100 text-emerald-800" : "bg-amber-100 text-amber-800"
                      }`}
                    >
                      WhatsApp: {linked ? "line live" : "pending"}
      ...
      {active && (
        <div className="rounded-2xl border border-grey-line bg-paper p-6 space-y-5">
          ...
          <div className="rounded-xl border border-grey-line bg-grey-light/50 p-4 space-y-1.5">
            <p className="font-body text-[11px] font-bold uppercase tracking-wider text-ink/50">
              Onboarding intake
            </p>
            <p className="font-body text-sm text-ink/80">
              Number plan:{" "}
              <strong>
                {active.phone_option
                  ? PHONE_OPTION_LABEL[active.phone_option] ?? active.phone_option
                  : "not chosen yet"}
              </strong>
              {active.phone_number ? ` — ${active.phone_number}` : ""}
              {active.phone_country ? ` (country: ${active.phone_country})` : ""}
