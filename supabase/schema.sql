...
-- 1) Ensure profiles table matches query fields
create table if not exists public.profiles (
  id uuid primary key references auth.users (id) on delete cascade,
  full_name text,
  business_name text,
  billing_region text not null default 'international' check (billing_region in ('international', 'pakistan')),
...
create table if not exists public.clients (
  ...
  phone_provisioning jsonb default '{}'::jsonb,
  timezone text,
  owner_whatsapp text,
...
-- 3) Create appointments table with BOTH time column variants to prevent query crashes
create table if not exists public.appointments (
  id uuid primary key default gen_random_uuid(),
  client_id uuid references auth.users (id) on delete cascade,
  customer_name text not null,
  appointment_time timestamptz not null default now(),
  start_time timestamptz not null default now(), -- Dual-support alias to prevent missing-column crashes
  status text not null default 'confirmed',
  timezone text,
  value numeric default 0,
  recovered_from_waitlist boolean default false,
...
-- 6) Create client_waitlist table (30-day active list)
...
-- Enable RLS and define open public select/update rules where applicable
alter table public.profiles enable row level security;
...
drop policy if exists "client_waitlist readable by owner" on public.client_waitlist;
create policy "client_waitlist readable by owner" on public.client_waitlist for all using (auth.uid() = client_id);